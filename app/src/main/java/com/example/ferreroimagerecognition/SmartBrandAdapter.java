...
public class SmartBrandAdapter extends RecyclerView.Adapter<SmartBrandAdapter.ViewHolder> {

    private final ArrayList<StoreVisionBo> storeVisionBolist;
    Context context;

    public SmartBrandAdapter(ArrayList<StoreVisionBo> storeVisionBolist){
...
//        ((ActivityManager) this.context.getSystemService(Context.ACTIVITY_SERVICE)).clearApplicationUserData();
    ...
    @Override
    public void onBindViewHolder( SmartBrandAdapter.ViewHolder viewHolder, int i) {
        viewHolder.textView1.setText(storeVisionBolist.get(i).getBrand());
